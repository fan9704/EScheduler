...
from src.models.pydantic.email_template import (
    EmailTemplateCreate, EmailTemplatePreview, EmailTemplateUpdate
)
...
class TestEmailTemplateService:
    ...
    async def test_create_template_success(self):
        ...
        with patch("src.models.tortoise.email_template.EmailTemplate.filter", return_value=mock_qs), \
                patch("src.models.tortoise.email_template.EmailTemplate.create", new_callable=AsyncMock,
    ...
    async def test_delete_template_success(self):
        ...
        with patch("src.models.tortoise.email_template.EmailTemplate.get_or_none", new_callable=AsyncMock) as mock_get:
    ...
    async def test_list_templates_with_filters(self):
        ...
        with patch("src.models.tortoise.email_template.EmailTemplate.all", return_value=mock_qs):
            result = await service.list_templates(is_active=True, search="Test", limit=10, offset=0)
    ...
    async def test_get_template_found(self):
        ...
        with patch("src.models.tortoise.email_template.EmailTemplate.get_or_none", AsyncMock(return_value=mock_t)):
    ...
    async def test_get_template_not_found_returns_none(self):
        service = await self.async_set_up_service()
        with patch("src.models.tortoise.email_template.EmailTemplate.get_or_none", AsyncMock(return_value=None)):
    ...
    async def test_update_template_success(self):
        ...
        with patch("src.models.tortoise.email_template.EmailTemplate.get_or_none",
    ...
    async def test_preview_template_jinja_template_error(self):
        service = await self.async_set_up_service()
        with patch.object(service, "_validate_template_syntax", side_effect=TemplateError("invalid")):
            with pytest.raises(ValidationError):
    ...
    async def test_preview_template_general_exception(self):
        ...
        with patch.object(service, "_render_template", side_effect=KeyError("Missing var")):
            with pytest.raises(ValidationError) as exc_info:
                await service.preview_template(
                    EmailTemplatePreview(
                        subject_template="Hi {{ name }}",
    ...
    async def test_get_template_usage_stats_success(self):
        ...
        mock_usage = [
            ...
            SimpleNamespace(used_at=dt.datetime.now() - dt.timedelta(days=1), template_id=1)
        ]
        ...
        with patch("src.models.tortoise.email_template.EmailTemplate.get_or_none", AsyncMock(return_value=mock_t)), \
                patch("src.models.tortoise.email_template.EmailTemplateUsage.filter", return_value=mock_usage_qs):
...
def test_validate_template_syntax_raises():
    ...
    with patch.object(service.jinja_env, "from_string", side_effect=TemplateError("bad")):
        with pytest.raises(ValidationError):
