...
class ScheduleHelperService {
	...
	async generateRateExpression(
		request: RateExpressionRequest,
	): Promise<ScheduleExpressionResponse> {
		console.log("API 調用 - 生成 Rate 表達式:", request);
		const response = await apiService.post<ScheduleExpressionResponse>(
			`${this.basePath}/rate`,
			request,
		);
	...
	async validateExpression(
		...
		const response = await apiService.post<ScheduleValidationResponse>(
	...
	async getTemplates(): Promise<ScheduleTemplateResponse[]> {
		const response = await apiService.get<ScheduleTemplateResponse[]>(
	...
	async getCronHelp(): Promise<CronHelpResponse[]> {
		const response = await apiService.get<CronHelpResponse[]>(
			`${this.basePath}/cron-help`,
		);
