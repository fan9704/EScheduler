...
async def trigger_task(
    ...
    try:
        ...
        if success:
            return JSONResponse(
                status_code=200, content={"message": "任務觸發成功", "task_id": task_id}
            )
        else:
            return JSONResponse(
                status_code=500, content={"message": "任務執行失敗", "task_id": task_id}
            )
    except HTTPException as e:
        return JSONResponse(
            status_code=e.status_code, content={"message": e.detail, "task_id": task_id}
        )
