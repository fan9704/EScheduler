...
class ScheduleHelperService:
    ...
    def __init__(self):
        self.quick_schedules = {
            ...
            "weekend_backup": {
                "expression": "cron(0 2 * * 0)",
                "description": "週日凌晨2點執行",
            },
            "monthly_report": {
    ...
    async def generate_rate_expression(
        self, request: RateExpressionRequest
    ) -> ScheduleExpressionResponse:
        """生成 Rate 表達式"""
        ...
        description = (
            f"每{request.value}{unit_zh.get(request.unit, request.unit)}執行一次"
        )
    ...
    async def generate_cron_expression(
        self, request: CronExpressionRequest
    ) -> ScheduleExpressionResponse:
        """生成 Cron 表達式"""
    ...
    async def generate_quick_schedule(
        self, request: QuickScheduleRequest
    ) -> ScheduleExpressionResponse:
        """生成快速排程表達式"""
        if request.type in self.quick_schedules:
            ...
            return ScheduleExpressionResponse(
                ...
                type=ScheduleType.CRON
                if template["expression"].startswith("cron")
                else ScheduleType.RATE,
                description=template["description"],
                next_runs=self._calculate_next_runs_from_expression(
                    template["expression"]
                ),
            )
        ...
        if request.type == "custom_time" and request.time:
            ...
            description = (
                f"每天{request.time}執行"
                if not request.weekdays
                else f"指定星期{request.time}執行"
            )
            ...
            return ScheduleExpressionResponse(
                ...
                type=ScheduleType.RATE,
                description=description,
                next_runs=self._calculate_next_runs_rate(
                    request.interval, request.unit
                ),
            )
    ...
    async def get_schedule_templates(self) -> List[ScheduleTemplateResponse]:
        ...
        templates = [
            ...
            {
                "name": "工作日下午5點",
                "expression": "cron(0 17 * * 1-5)",
                "category": "工作日排程",
                "description": "週一到週五下午5點執行",
            },
            {
                "name": "工作日每小時",
                "expression": "cron(0 * * * 1-5)",
                "category": "工作日排程",
        ...
        return [
            ScheduleTemplateResponse(
                ...
                type=ScheduleType.CRON
                if t["expression"].startswith("cron")
                else ScheduleType.RATE,
            )
            for t in templates
        ]
    ...
    def _calculate_next_runs_rate(
        self, value: int, unit: str, count: int = 5
    ) -> List[str]:
        """計算 Rate 表達式的接下來執行時間"""
    ...
    def _generate_cron_description_from_parts(self, parts: List[str]) -> str:
        ...
        if weekday != "*":
            ...
            if "-" in weekday:
                ...
                desc_parts.append(
                    f"{weekday_map.get(start, start)}到{weekday_map.get(end, end)}"
                )
            elif "," in weekday:
