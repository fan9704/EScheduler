...
class StatisticService:
    ...
    async def get_dashboard_metrics(self):
        ...
        enabled_tasks = await self.task_repository.count_tasks_by_state(
            TaskState.ENABLED
        )
        disabled_tasks = await self.task_repository.count_tasks_by_state(
            TaskState.DISABLED
        )
        today_executions_count = (
            await self.execution_repository.count_today_executions()
        )
