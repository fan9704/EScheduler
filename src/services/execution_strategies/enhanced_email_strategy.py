...
class EnhancedEmailExecutionStrategy(ExecutionStrategy):
    """增強版 Email 發送執行策略，支援模板功能"""
    ...
    async def execute(self, target_arn: str, target_input: Dict[str, Any]) -> ExecutionResult:
        ...
        try:
            ...
            if use_template and template_id:
                # 使用模板發送
                result = await self._send_with_template(target_arn, target_input, start_time)
            else:
                # 直接發送
                result = await self._send_direct(target_arn, target_input, start_time)
    ...
    async def _send_with_template(self, target_arn: str, target_input: Dict[str, Any], start_time: float) -> ExecutionResult:
        """使用模板發送郵件"""
        template_id = target_input.get('template_id')
        ...
        # 渲染模板
        try:
            rendered_subject = self._render_template(template.subject_template, template_variables)
            rendered_body = self._render_template(template.body_template, template_variables)
            rendered_html = None
            if template.html_template:
                rendered_html = self._render_template(template.html_template, template_variables)
        ...
        sender = target_input.get('sender', template.default_sender or self.smtp_username or 'noreply@example.com')
    ...
    async def _send_direct(self, target_arn: str, target_input: Dict[str, Any], start_time: float) -> ExecutionResult:
        """直接發送郵件（不使用模板）"""
        recipients = self._parse_recipients(target_arn, target_input)
    ...
    async def _send_email(self, recipients: List[str], cc: List[str], bcc: List[str],
                         sender: str, subject: str, body: str, html_body: Optional[str] = None) -> Dict[str, Any]:
        """實際發送郵件的方法"""
        try:
    ...
    async def _create_usage_record(self, template: EmailTemplate, variables_used: Dict[str, Any],
                                 rendered_subject: str, rendered_body: str, rendered_html: Optional[str],
                                 recipients: List[str], cc_recipients: List[str], bcc_recipients: List[str],
                                 sender: str, is_successful: bool, error_message: Optional[str] = None) -> EmailTemplateUsage:
        """創建模板使用記錄"""
...
# 為了向後兼容，保留原有的 EmailExecutionStrategy
class EmailExecutionStrategy(EnhancedEmailExecutionStrategy):
    """Email 發送執行策略（向後兼容版本）"""
